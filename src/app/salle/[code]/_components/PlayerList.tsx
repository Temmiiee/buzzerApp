    ...
    <TooltipProvider>
      <ul className="space-y-3">
        {state.players.map((player) => {
          ...
          const isAdmin = state.isAdmin && state.user.id === player.id;
          const isDesignated = state.config.mode === 'single_buzz' && state.config.designatedPlayerId === player.id;

          return (
            <li
              key={player.id}
              className={`flex items-center justify-between p-3 rounded-lg transition-all duration-300 ${isWinner ? 'bg-primary/20 scale-105 shadow-lg' : 'bg-muted/50'}`}
            >
